...
def stochastic_loss_fn(stl_class, current_traj, log_prob_seq, control_seq, ref_traj,
    ...
    stl_robustness = stl_class.compute_robustness(current_traj).squeeze()
    stl_reward = - torch.relu(-stl_robustness + margin)
    ...
    return loss, stl_loss, ref_loss, control_loss, stl_robustness
...
def deterministic_loss_fn(stl_class, current_traj, log_prob_seq, control_seq, ref_traj, 
    ...
    stl_robustness = stl_class.compute_robustness(current_traj).squeeze()
    stl_loss = torch.relu(-stl_robustness + margin)
...
def stochastic_loss_fn_multi_agent_ego(stl_class, ego_traj, opp_traj, log_prob_seq_ego, log_prob_seq_opp, 
    ...
    stl_robustness = stl_class.compute_robustness(ego_traj, opp_traj).squeeze()
    stl_reward = - torch.relu(-stl_robustness + margin)
...
def deterministic_loss_fn_multi_agent_ego(args, stl_class, ego_traj, opp_traj):
    ...
    stl_robustness = stl_class.compute_robustness_ego(ego_traj, opp_traj).squeeze()
    stl_loss = torch.relu(-stl_robustness + args.robust_margin)
    ref_loss = torch.nn.MSELoss()(ego_traj, ref_traj_ego)
    loss = stl_loss + args.ref_loss_weight*ref_loss

    return loss, stl_loss, ref_loss, None, stl_robustness
...
def deterministic_loss_fn_multi_agent_opp(args, stl_class, ego_traj, opp_traj):
    ...
    stl_robustness = stl_class.compute_robustness_opp(ego_traj, opp_traj).squeeze()
    stl_loss = - torch.relu(-stl_robustness + args.robust_margin)
...
class Trainer:
    ...
    def train(self, args, save_path_ego=None, save_path_opp=None, whose_turn="ego", iteration=0):        
        ...
        loss_ego, stl_loss, ref_loss, _, stl_robustness, loss_opp = self.rollout(
                                                                args=args,
                                                                policy_ego_set=self.policy_avg_ego_set,
                                                                policy_opp_set=self.policy_avg_opp_set,
                                                                whose_turn=None,
        ...
        for epoch in range(args.train_epoch):
            ...
            loss.backward()
            if epoch % 50 == 0:
                print(f"Iteration {iteration}, agent {whose_turn}, epoch {epoch}, stl loss {stl_loss.mean():.3f}, ref loss {ref_loss.mean():.3f}, stl robustness {stl_robustness.mean():.3f}")
    ...
    def rollout(self, args, policy_ego_set, policy_opp_set, whose_turn, policy_sample_dist):
        ...
        loss_ego, stl_loss, ref_loss, control_loss, stl_robustness = loss_fn_ego(
        ...
        return loss_ego, stl_loss, ref_loss, control_loss, stl_robustness, loss_opp
