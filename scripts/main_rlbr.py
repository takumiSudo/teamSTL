...
sys.path.append("./")
from utils.modules_helper import LSTM, RecurrentAgent
from utils.dynamic_helper import (
    single_integrator,
    double_integrator,
    kinematic_model,
    double_integrator_3d,
    rollout_batch_two_agents,
)
from utils.dynamic_helper import quadrotor
from utils.stl_helper import (
    STLFormulaReachAvoidTwoAgents,
    STLFormulaReachAvoidTwoAgents3D,
)
from utils.train_helper import TrainerReccurentPPO
import argparse
import copy
...
def main(args):
    ...
    if args.dynamic == "dt3d" or args.dynamic == "rotor":
        stl_class = STLFormulaReachAvoidTwoAgents3D(
            obs, goal, T, zone1, zone2, altitude1, altitude2
        )
    else:
        stl_class = STLFormulaReachAvoidTwoAgents(obs_1, obs_2, obs_3, obs_4, goal, T)

    # current best response policy for ego and opponent
    policy_br_ego = RecurrentAgent(
        hiddent_dim, 2 * args.state_dim, args.control_dim, args.action_max
    )
    policy_br_opp = RecurrentAgent(
        hiddent_dim, 2 * args.state_dim, args.control_dim, args.action_max
    )

    # initial (random) policies for ego and opponent
    policy_init_ego = RecurrentAgent(
        hiddent_dim, 2 * args.state_dim, args.control_dim, args.action_max
    )
    policy_init_opp = RecurrentAgent(
        hiddent_dim, 2 * args.state_dim, args.control_dim, args.action_max
    )
    ...
    ## if we want to load existing models
    # checkpoint_ego = torch.load(model_filename_ego+'.pth', weights_only=True)
    # policy_init_ego.load_state_dict(checkpoint_ego['model_state_dict'])

    # checkpoint_opp = torch.load(model_filename_opp+'.pth', weights_only=True)
    # policy_init_opp.load_state_dict(checkpoint_opp['model_state_dict'])

    # build the trainer (without using supervised learning for average policy)
    trainer = TrainerReccurentPPO(
        args=args,
        stl_class=stl_class,
        policy_avg_ego=[policy_init_ego],
        policy_avg_opp=[policy_init_opp],
        p_sample_dist=policy_avg_sample_dist,
        policy_br_ego=policy_br_ego,
        policy_br_opp=policy_br_opp,
    )

    wandb.init(project="stl_game", config=vars(args))

    # train
    if not args.only_test:

        exploitability_list = np.zeros(
            fsp_iteration
        )  # store explotability over FSP iterations

        # fictitious self play loop
        for fsp_iteration_index in range(fsp_iteration):
            for agent_name in ["ego", "opp"]:
                # policy for exploitability calculation, optimized with stlcg
                exp_policy = LSTM(
                    hidden_dim=hiddent_dim,
                    pred_length=pred_len,
                    state_dim=2 * args.state_dim,
                    action_dim=args.control_dim,
                    stochastic_policy=False,
                    batch_first=args.batch_first,
                    action_max=args.action_max,
                )
                exploitability, p_sample = trainer.train(
                    args,
                    exp_policy,
                    iteration=fsp_iteration_index,
                    save_path_ego=model_filename_ego,
                    save_path_opp=model_filename_opp,
                    whose_turn=agent_name,
                )
                if agent_name == "opp":
    ...
    with torch.no_grad():
        ...
        for fsp_iteration_index in range(fsp_iteration):
            ...
            checkpoint_ego = torch.load(
                model_filename_ego + f"_best_iter{fsp_iteration_index}.pth",
                weights_only=True,
            )
            checkpoint_opp = torch.load(
                model_filename_opp + f"_best_iter{fsp_iteration_index}.pth",
                weights_only=True,
            )
        ...
        for test_index in range(test_batch_size):
            ...
            # policy_opp = opp_model_list[10] # to test different opponent policy, to demonstrate to robustness of Nash policy
        ...
        robustness = stl_class.compute_robustness_ego(
            current_traj_ego[..., :traj_dim], current_traj_opp[..., :traj_dim]
        ).squeeze()
        print(
            f"Test STL satisfaction num {torch.where(robustness>=0.)[0].shape[0]} out of {robustness.shape[0]} trajectories."
        )
        print(f"Test STL robustness mean {torch.mean(robustness).item()}.")
        test_index_list = [1, 2, 3, 4, 5]
        for test_index in test_index_list:
            stl_class.animate(
                current_traj_ego[test_index, ...],
                current_traj_opp[test_index, ...],
                name=args.fig_name + str(test_index),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--only_test", action=argparse.BooleanOptionalAction, help="Dont train policy"
    )
    parser.add_argument(
        "--fig_name", type=str, default="test_multi_agent_rlbr", help="Figure name"
    )
    parser.add_argument(
        "--dynamic", type=str, default="st", help="Single integrator or double"
    )
    parser.add_argument(
        "--total_time_step", type=int, default=51, help="Agents simulation steps"
    )
    parser.add_argument("--num_rollout_per_epoch", type=int, default=15)
    parser.add_argument("--anneal_lr", default=True, action="store_false")
    parser.add_argument(
        "--lr", type=float, default=1e-3, help="Best response training learn rate"
    )
    parser.add_argument(
        "--batch_size", type=int, default=1, help="Data collecting batch size"
    )
    ...
    parser.add_argument("--norm_adv", default=True, action="store_false")
    parser.add_argument("--clip_coef", type=float, default=0.2)
    parser.add_argument("--clip_vloss", default=True, action="store_false")
    parser.add_argument("--ent_coef", type=float, default=0.0)
    parser.add_argument("--vf_coef", type=float, default=0.5)
    parser.add_argument("--max_grad_norm", type=float, default=0.5)
    parser.add_argument("--target_kl", type=float, default=None)
    parser.add_argument(
        "--train_epoch", type=int, default=201, help="Training epochs num"
    )
    parser.add_argument(
        "--robust_margin", type=float, default=0.1, help="STL robustness margin"
    )
    parser.add_argument(
        "--ref_loss_weight",
        type=float,
        default=0.2,
        help="Loss weight of following the ref traj",
    )
    parser.add_argument(
        "--batch_first", default=True, action=argparse.BooleanOptionalAction
    )
    parser.add_argument(
        "--batch_data_gen_fn",
        default=rollout_batch_two_agents,
    ...
    if args.dynamic == "st":
        ...
        state_dim, control_dim, traj_dim, action_max = (
            5,
            2,
            2,
            torch.tensor([0.3, 7.5]).float(),
        )
        from config import *
        ...
        state_dim, control_dim, traj_dim, action_max = (
            ...
            3,
            3,
            torch.tensor([30 * (math.pi / 180), 30 * (math.pi / 180), 0.15]).float(),
        )
        from config_3d import *
    else:
    ...
    parser.add_argument(
        ...
        type=list,
        default=ref_X_ego,
        help="Ego task-unaware reference trajectory",
    )
    parser.add_argument(
        "--ref_opp",
        type=list,
        default=ref_X_opp,
        help="Opponent task-unaware reference trajectory",
    )
